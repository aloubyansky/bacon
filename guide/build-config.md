...
The `BOM` strategy exposes an option to capture the resolved artifacts with the `redhat` version qualifier and generate a Cachi2 Lock File that can be used later to prefetch the generated MRRC content into a Konflux pipeline for container image building or an MRRC release process.
